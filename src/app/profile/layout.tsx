  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gray-100">
      ...
      <aside
        ...
        className="hs-overlay [--auto-close:lg] hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform w-64 h-full fixed inset-y-0 start-0 z-[60] bg-white border-e border-gray-200 shadow-lg lg:relative lg:translate-x-0 lg:inset-0"
        role="dialog"
        tabIndex={-1}
        aria-label="Sidebar"
      >
        <div className="flex flex-col h-full p-6">
          ...
          <nav className="space-y-4">
            {filteredNavItems.map(({ href, icon: Icon, text }) => (
              <Link
                key={href}
                href={href}
                className="flex items-center gap-x-3 py-2 px-3 text-sm font-medium text-gray-700 rounded-lg hover:bg-gray-100 hover:text-gray-900 transition-all duration-200"
              >
